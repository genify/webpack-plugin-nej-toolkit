...
* 标记为大小写不敏感单词，并以 “@” 为前缀，如 “@STYLE” 或者 “@style”
* 成对出现的标记必须使用结束标记，结束标记为开始标记的 “@” 符号前加 “/”，如 “/@NOPARSE” 作为 “@NOPARSE” 的结束标记
...
| [UMI](#umi) | 指定UMI单页模块配置信息以及版本输出位置 |
| [MODULE](#module) | 标记UMI单页模块预内联模块 |
| [TEMPLATE](#template) | 指定UMI单页模块的模板插入位置 |
...
此标记用以表明当前位置插入打包后的样式文件，打包工具自动识别 define.js 文件为 NEJ 模块依赖系统文件

使用NEJ依赖系统，后续第一个外联脚本为 define.js ：
...
此标记用以表明当前位置插入UMI模块版本信息，后续脚本内仅允许出现模块的版本配置信息，否则打包后内容将丢失，如果项目没有使用NEJ的模块调度模型则可以忽略此标记
...
```html
<div style="display:none;" id="template-box-1">
  <!-- @TEMPLATE -->
  <textarea name="txt" id="txt-0">
      text content
  </textarea>
...
<div style="display:none;" id="template-box-2">
  <!-- @TEMPLATE -->
  <textarea name="css" data-src="./css/component/left.css"></textarea>
  <textarea name="js" data-src="./javascript/component/left.js"></textarea>
  <textarea name="html" data-src="./html/component/left.html"></textarea>
